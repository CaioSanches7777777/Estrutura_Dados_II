...
void imprimeGrafoEmOrdem(struct Node* raiz, int nivel_NO) {
    int i;
    ...
    if (raiz != NULL) {
        if (raiz->right != NULL) {
            imprimeGrafoEmOrdem(raiz->right, nivel_NO + 1);
            printf("\r");
            printf("\n");
        ...
        if (raiz->left != NULL) {
            ...
            printf("\b\\\n");
            imprimeGrafoEmOrdem(raiz->left, nivel_NO + 1);
        }
    }
}
